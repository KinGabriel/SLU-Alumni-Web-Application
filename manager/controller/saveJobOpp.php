...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (isset($_FILES['image']) && $_FILES['image']['error'] === UPLOAD_ERR_OK) {
            ...
            if ($image_data === false) {
                echo "<script>alert('Failed to read image data.'); window.location.href = '../view/jobOpportunity.php';</script>";
            ...
            if (isset($_FILES['image'])) {
                ...
                if ($upload_error != UPLOAD_ERR_OK) {
                    echo "<script>alert('Image upload error: $upload_error'); window.location.href = '../view/jobOpportunity.php';</script>";
                    exit;
                }
            } else {
                echo "<script>alert('No image uploaded.'); window.location.href = '../view/jobOpportunity.php';</script>";
                exit;
            }
        }
        ...
        if ($stmt = $conn->prepare($sql)) {
            ...
            if ($stmt->execute()) {
                echo "<script>
                        ...
                        window.location.href = '../view/jobOpportunity.php'; 
                      </script>";
            } else {
                echo "<script>
